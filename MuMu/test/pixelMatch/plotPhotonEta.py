...
h_temp = TH1F("h_temp", "", 60, -3, 3)
h_temp.SetTitle("")
h_temp.SetStats(0)
h_temp.GetYaxis().SetTitle("Events / 0.1")
h_temp.GetXaxis().SetTitle("#eta^{#gamma}")


tree["data"].Draw("phoEta>>h_temp", selection)
hdata = h_temp.Clone(h_temp.GetName() + "_data")

tree["z"].Draw("phoEta>>h_temp", selection)
hmc = h_temp.Clone(h_temp.GetName() + "_mc")

tree["z"].Draw("phoEta>>h_temp", "(%s) && !isFSR" % (selection) )
...
hmc.Draw()
hmc.GetYaxis().SetRangeUser(0, 300)
