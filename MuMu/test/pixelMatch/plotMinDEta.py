...
h_temp.GetXaxis().SetTitle("min |#Delta #eta(#mu^{#pm},#gamma)|")


tree["data"].Draw("minDEta>>h_temp", selection)
hdata = h_temp.Clone(h_temp.GetName() + "_data")

tree["z"].Draw("minDEta>>h_temp", selection)
hmc = h_temp.Clone(h_temp.GetName() + "_mc")

tree["z"].Draw("minDEta>>h_temp", "(%s) && !isFSR" % (selection) )
