...
h_DEta = TH1F("h_DPhi_data_eb", "min #Delta #phi (#mu, #gamma)", len(xbins)-1, array("d", xbins))

totSelection = "phoIsEB & abs(mmgMass-90)<15"
tree["data"].Draw("minDPhi>>" + h_DEta.GetName(), totSelection)
htot = h_DEta.Clone(h_DEta.GetName() + "_tot")

passSelection = totSelection + " & (%s)" % vetoCut
tree["data"].Draw("minDPhi>>" + h_DEta.GetName(), passSelection)
...
geff.GetYaxis().SetTitle("GSF Electron Veto Efficiency")
...
c1.SetName( "drToTrk1Eff_vs_DPhi_data_EB" )
c1.SetGridx()
c1.SetGridy()
c1.Update()
...
h_DEta = TH1F("h_DPhi_mc_eb", "min #Delta #phi (#mu, #gamma)", len(xbins)-1, array("d", xbins))

totSelection = "phoIsEB & abs(mmgMass-90)<15"
tree["z"].Draw("minDPhi>>" + h_DEta.GetName(), totSelection)
htot = h_DEta.Clone(h_DEta.GetName() + "_tot")

passSelection = totSelection + " & (%s)" % vetoCut
tree["z"].Draw("minDPhi>>" + h_DEta.GetName(), passSelection)
hpass = h_DEta.Clone(h_DEta.GetName() + "_pass")
...
latexLabel.DrawLatex(0.7, 0.2, "Barrel")
c1.SetName( "drToTrk1Eff_vs_DPhi_MC_EB" )
c1.SetGridx()
c1.SetGridy()
c1.Update()
...
h_DEta = TH1F("h_DPhi_data_ee", "min #Delta #phi (#mu, #gamma)", len(xbins)-1, array("d", xbins))
...
c1.SetName( "drToTrk1Eff_vs_DPhi_data_EE" )
c1.SetGridx()
c1.SetGridy()
c1.Update()
...
h_DEta = TH1F("h_DPhi_mc_ee", "min #Delta #phi (#mu, #gamma)", len(xbins)-1, array("d", xbins))
...
latexLabel.DrawLatex(0.75, 0.96, "Summer11")
latexLabel.DrawLatex(0.7, 0.2, "Endcaps")
c1.SetName( "drToTrk1Eff_vs_DPhi_MC_EE" )
c1.SetGridx()
c1.SetGridy()
c1.Update()
...
#tree["z"].Draw("minDPhi>>h_DEta", "isEB & abs(mmgMass-90)<15")
#h_DPhi_z_tot = h_DEta.Clone("h_DPhi_z_tot")

#tree["z"].Draw("minDPhi>>h_DEta", "isEB & abs(mmgMass-90)<15 & !phoHasPixelMatch")
#h_DPhi_z_pass = h_DEta.Clone("h_DPhi_z_pass")
