...
#outputFileName = "yuriisMethod-Powheg_Fall10.txt"
# outputFileName = "yuriisMethod-qcd_Fall10.txt"
# outputFileName = "yuriisMethod-tt_Fall10.txt"
outputFileName = "yuriisMethod-dummy_Fall10.txt"
#outputFileName = "yuriisMethod-data_Nov4ReReco.txt"

outputExpression = "mass[mm]:mmgMassVCorr:phoPt[g]:phoEta[g]"
...
lyonCuts = [
    ...
    "muPt[mu1] > 10",
    "muPt[mu2] > 10",
    "phoPt[g] > 10",  ## comment out for input to es/res fits
    "40 <= mass[mm] & mass[mm] <= 80", ## originally [40,80]
    "mmgDeltaRNear <= 0.8",
    "87.2 <= mmgMass && mmgMass <= 95.2",  ## comment out for input to es/res fits
    "60 <= mmgMassVCorr && mmgMassVCorr <= 120",
...
triggerCutData = [
    "(133874 <= run & run <= 147195 & muPt[mu1] > 20 & muHltMu9Match[mu1]) | " +\
    "(133874 <= run & run <= 147195 & muPt[mu2] > 20 & muHltMu9Match[mu2]) | " +\
    "(147196 <= run & run <= 148821 & muPt[mu1] > 20 & muHltMu11Match[mu1]) | " +\
    "(147196 <= run & run <= 148821 & muPt[mu2] > 20 & muHltMu11Match[mu2]) | " +\
    "(148822 <= run & run <= 149442 & muPt[mu1] > 20 & muHltMu15v1Match[mu1]) |" +\
    "(148822 <= run & run <= 149442 & muPt[mu2] > 20 & muHltMu15v1Match[mu2])"
    ]

triggerCutMC = [
    "(muPt[mu1] > 20 & muHltMu9Match[mu1]) | " +\
    "(muPt[mu2] > 20 & muHltMu9Match[mu2])"
    ]
...
#selection = makeSelection(lyonCuts + fsrCuts)
selection = makeSelection(lyonCuts)
#selection = makeSelection(lyonCuts + triggerCutData)
#selection = makeSelection(lyonCuts + triggerCutMC)
