...
selection = "m2 < 80 & 30 < pt & pt < 9999 & r9 > 0.94 & abs(eta)<1.5"
labels = ["Barrel", "R_{9} > 0.94", "E_{T}^{#gamma} 15-20 GeV"]
#selection = "abs(eta)>1.5 & m2 < 80"
scale = 1.
xRange = (-4.5, 4.5) ## 2011A, 715/pb, July17 JSON
...
#c.SetGridy()
gr.GetXaxis().SetTitle("Photon Energy Scale (%)")
gr.GetYaxis().SetTitle("- #Delta log L  for m_{#mu#mu#gamma}")
...
for i in range(len(labels)):
    latexLabel.SetText(0.21, 0.35 + 0.075 * i, labels[-1-i])
    latexLabels.append(latexLabel.DrawClone())
