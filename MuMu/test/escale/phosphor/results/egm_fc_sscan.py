...
bincenters = [str(-5.0 + 0.5 * i) for i in range(21)]
...
def make_list_of_sources(jobname_template):
    ...
    for sfit in bincenters:
        ## The job name encodes the pt, category and data/MC
        jobname = jobname_template + '_sfit' + sfit
...
#______________________________________________________________________________
def xyasymmerrors_var_vs_var_getter_factory(xvarname, yvarname):
    """
    Returns a tupler of 6 functions that take a workspace and return
    x, y, exl, exh, eyl, eyh where both x and y correspond to the values 
    and asymmetric error of workspace variables of the given names 
    """
    xgetter  = value_getter_factory(xvarname)
    exlgetter = strue_low_error_getter_factory(xvarname)
    exhgetter = strue_high_error_getter_factory(xvarname)
    ygetter  = value_getter_factory(yvarname)
    eylgetter = sfit_low_error_getter_factory(yvarname)
    eyhgetter = sfit_high_error_getter_factory(yvarname)
    return (xgetter, ygetter, exlgetter, exhgetter, eylgetter, eyhgetter)
## End of xyasymmerrors_var_vs_var_getter_factory(xvarname, yvarname)
...
for cat in categories:
    ...
    cfg = Config(
        ## Canvas name
        name = 'egm_fc_sscan_%s' % (cat.name,), 
        ## Canvas title
        title = ', '.join(cat.labels),
        ## Axis titles
        xtitle = 'True E^{#gamma} Scale (%)',
        ytitle = 'Fit E^{#gamma} Scale (%)',
    ...
    for title, version in [('Jan12 rereco + Rochcor', 'yyv5'),
        ...
        cfg.getters.append(
            xyasymmerrors_var_vs_var_getter_factory('phoScaleTrue',
                                                    'phoScale')
            )
