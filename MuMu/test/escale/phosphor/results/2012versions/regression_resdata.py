...
of (EB, EE) x (low R9, high R9) x (low PU, high PU) for real-data fit
...
from JPsi.MuMu.scaleFitter import subdet_r9_nv_categories
from JPsi.MuMu.scaleFitter import subdet_nv_categories
...
categories = list(subdet_nv_categories) + list(subdet_r9_categories) + list(subdet_r9_nv_categories)

binedges  = list(BinEdges([10, 12, 15, 20, 25, 35, 50]))
binedges2 = list(BinEdges([10, 12, 15, 20, 25, 35, 999]))


# from JPsi/MuMu/test/scale/phosphor/results/et_bins.py
bincenters = [11.025496837677762, 13.381777593207669, 17.167009825347275, 22.20578901298628, 28.616177561337238, 43.315006642689276]
binlowerrors = [0.66537410873147174, 0.95516588223967069, 1.5426724715404649, 1.5513259204627659, 2.6303617600898193, 8.3933265622878466]
binhigherrors = [0.65474544971813842, 1.0661899747371528, 1.8380466543959919, 1.8288390379927328, 3.7618379176649412, 18.067378219167722]
...
for cat in categories:
    ...
    for title, version in [('2012', 'sixie'),
                           ]:        
        ...
        labels.append('PtLow_{lo}_PtHigh_{hi}')
...
def main():
    ...
    save_graphs_to_file(plotters, 'pt_res_data.root')
## End of main(..)
