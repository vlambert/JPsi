...
from JPsi.MuMu.common.roofit import ConditionalObservables
...
##                                            {phoF, phoScale})''')
phoMean = w.factory('''FormulaVar::phoMean("phoFFunc * phoScale / 100.",
                                           {phoFFunc, phoScale})''')
...
phoWidth = w.factory('''FormulaVar::phoWidth(
    "phoFFunc * phoRes / {effSigma}",
    {{phoFFunc, phoRes[{effSigma},0.1,1000]}}
    )'''.format(effSigma = phoEResMC))
