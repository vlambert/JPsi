...
cfgs = [
    ...
    Config(
        ...
        sreco_snapshots2 = ['sFit_sreco_mc_cbShape_mmMass90_EB_lowR9_PhoEt%d-%d'
                            % (lo, hi) for lo, hi in binedges],
        ## MC truth scale
        strue_snapshots = ['sFit_strue_mc_bifurGauss_mmMass90_EB_lowR9_'
    ...
    Config(
        ...
        sreco_snapshots2 = ['sFit_sreco_mc_cbShape_mmMass90_EB_highR9_PhoEt%d-%d'
                           % (lo, hi) for lo, hi in binedges],
        ## MC truth scale
        strue_snapshots = ['sFit_strue_mc_bifurGauss_mmMass90_EB_highR9_'
                           'PhoEt%d-%d' % (lo, hi) for lo, hi in binedges],
    ),
    ###########################################################################
    ...
    Config(
        ...
        sreco_snapshots2 = ['sFit_sreco_mc_cbShape_mmMass90_EE_lowR9_PhoEt%d-%d'
                           % (lo, hi) for lo, hi in binedges],
        ## MC truth scale
        strue_snapshots = ['sFit_strue_mc_bifurGauss_mmMass90_EE_lowR9_'
                           'PhoEt%d-%d' % (lo, hi) for lo, hi in binedges],
    ),
    ###########################################################################
    ## EE, R9 > 0.95, mmMass < 80 GeV, mmgMass in [87.2, 95.2]
...
workspaces = ['ws1'] * n
snapshot = 'chi2_strue_mc_bifurGauss_mmMass90_%s_PhoEt%d-%d_iter0'
