...
filename4 = 'mc_sreco_strue_Baseline_mod4.root'
...
cfgs = [
    ...
    Config(
        ...
        sreco_snapshots1 = ['sFit_sreco_mc_cbShape_mmMass80_EB_highR9_PhoEt%d-%d'
                           % (lo, hi) for lo, hi in binedges],
        sreco_snapshots4 = ['sFit_sreco_mc_cbShape_mmPlusMmgMass190_EB_highR9_PhoEt%d-%d'
                           % (lo, hi) for lo, hi in binedges],
    ...
    Config(
        ...
        sreco_snapshots1 = ['sFit_sreco_mc_cbShape_mmMass80_EE_lowR9_PhoEt%d-%d'
                           % (lo, hi) for lo, hi in binedges],
        sreco_snapshots4 = ['sFit_sreco_mc_cbShape_mmPlusMmgMass190_EE_lowR9_PhoEt%d-%d'
                           % (lo, hi) for lo, hi in binedges],
    ...
    Config(
        ...
        sreco_snapshots1 = ['sFit_sreco_mc_cbShape_mmMass80_EE_highR9_PhoEt%d-%d'
                           % (lo, hi) for lo, hi in binedges],
        sreco_snapshots4 = ['sFit_sreco_mc_cbShape_mmPlusMmgMass190_EE_highR9_PhoEt%d-%d'
                           % (lo, hi) for lo, hi in binedges],
...
################################################################################
## Plot the p-values of the reco s-Fits fits, m(yy) + m(uuy) < 190 GeV
filenames = [filename4] * n
workspaces = ['ws1'] * n
snapshot = 'chi2_sreco_mc_cbShape_mmPlusMmgMass190_%s_PhoEt%d-%d_iter0'
...
canvases.next('sreco_pvalues_vs_phoEt_mmPlusMmgMass190')
frp.plotall(logy = True, title = 's_{reco} Fits')

## Make the distribution of the p-values
hist = frp.histogramall(
    name = 'h_sreco_pvalues_mmPlusMmgMass190',
    title = 's_{reco} = E^{#gamma}_{reco}/E^{#gamma}_{kin};p-value;Fits',
    nbins = 5, xlow = 0, xhigh = 1
    )
c1 = canvases.next('sreco_pvalues_distro_mmPlusMmgMass190')
hist.Draw('e0')
plotters.append(hist)
