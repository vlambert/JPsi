...
fit = struefit.clone()
fit.applyDefinitions([Model('cbShape'), subdet_r9_categories[1], PhoEtBin(30, 100)])
fit.name = fit.name + '_0p99containment'
fit.labels.insert(1, '99% Containment')
fit.fitRangeFraction = 0.99
fit.pdf = 'cbShape_an'
struefits.append(fit)

fit = struefit.clone()
fit.applyDefinitions([Model('gauss'), subdet_r9_categories[1], PhoEtBin(30, 100)])
fit.name = fit.name + '_0p70containment'
fit.labels.insert(1, '70% Containment')
fit.fitRangeFraction = 0.70
fit.pdf = 'gauss_an'
struefits.append(fit)

fit = struefit.clone()
fit.applyDefinitions([Model('cbShape'), subdet_r9_categories[2], PhoEtBin(10, 12)])
fit.name = fit.name + '_0p70containment'
fit.labels.insert(1, '70% Containment')
fit.fitRangeFraction = 0.70
fit.pdf = 'cbShape_an'
...
fit = struefit.clone()
fit.applyDefinitions([Model('gauss'), subdet_r9_categories[2], PhoEtBin(10, 12)])
fit.name = fit.name + '_0p70containment'
fit.labels.insert(1, '70% Containment')
fit.fitRangeFraction = 0.70
fit.pdf = 'gauss_an'
...
fit = struefit.clone()
fit.applyDefinitions([Model('bifurGauss'), subdet_r9_categories[2], PhoEtBin(10, 12)])
fit.name = fit.name + '_0p70containment'
fit.labels.insert(1, '70% Containment')
fit.fitRangeFraction = 0.70
fit.pdf = 'bifurGauss_an'
struefits.append(fit)
...
for plot in _fits:
    ...
    for i in range (plot.niter-1, plot.niter):
        ...
        if plot.pdf in ['gauss_an', 'cbShape_an']:
            varname = '#mu'
        print '%6.2f +/- %4.2f' % ( ws1.var(varname).getVal(),
                                    ws1.var(varname).getError() ),
