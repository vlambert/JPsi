 ...
 *
 * $Id: fitJPsi2SMassUnbinned.C,v 1.4 2010/07/25 16:53:30 veverka Exp $
 *
 *
...
fitJPsi2SMassUnbinned(const char *filename = "JPsi2SMassOS_186nb.txt",
  const char* plotOpt = "NEU",
  const int nbins = 25,
  const char* filenameB = "JPsi2SMassSS_186nb.txt")
{
  ...
  double minMass = 3.4;
  double maxMass = 4.0;
  RooRealVar  mass("mass","M^{#mu#mu}", minMass, maxMass,"GeV/c^{2}");
...
//  Parameters for a Gaussian and a Crystal Ball Lineshape
  RooRealVar  m0   ("m_{0}", "Bias", 3.7, minMass, maxMass,"GeV/c^{2}");
  RooRealVar  sigma("#sigma","Width", 0.05,0.01,0.1,"GeV/c^{2}");
...
// Plot 1
  ...
  tex->SetLineWidth(2);
  tex->Draw();
  tex->DrawLatex(0.2, 0.725, "7 TeV Data, L = 186 pb^{-1}");
