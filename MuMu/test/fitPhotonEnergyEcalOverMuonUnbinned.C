 ...
 *
 * $Id: fitPhotonEnergyEcalOverMuonUnbinned.C,v 1.1 2010/09/10 11:35:07 veverka Exp $
 *
 *
...
RooGaussian fitPhotonEnergyEcalOverMuonUnbinned(const char *filename = "phoEnergyEcalOverMuon.txt",
  ...
  const int nbins = 10)
{
  ...
  double minMass = 0;
  double maxMass = 2;
  RooRealVar  mass("mass","E_{#gamma)}(ECAL) / E_{#gamma}(#mu kinematics)", minMass, maxMass);
...
//  Parameters for a Gaussian and a Crystal Ball Lineshape
  RooRealVar  m0   ("m_{0}", "Bias", 1.5, minMass, maxMass);
  RooRealVar  sigma("#sigma","Width", 0.1,0.005,1.0);
